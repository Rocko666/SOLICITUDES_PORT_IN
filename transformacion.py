...
@seguimiento_transformacion
def fun_controlpoint1(sqlContext, val_fecha_ejecucion):
    
    print(msg_succ('================================================================'))
    print(msg_succ('======== FUNCION DE CONTROL => fun_controlpoint1 ==============='))
    print(msg_succ('================================================================'))
    
    df_otc_t_r_om_portin_co, val_str_df_otc_t_r_om_portin_co= fun_cargar_df_otc_t_r_om_portin_co(sqlContext, val_fecha_ejecucion)  
    ...
    df_otc_t_r_boe_sales_ord, val_str_df_otc_t_r_boe_sales_ord = fun_cargar_df_otc_t_r_boe_sales_ord(sqlContext, val_fecha_ejecucion)
    
    df_otc_t_r_boe_ord_item, val_str_df_otc_t_r_boe_ord_item = fun_cargar_df_otc_t_r_boe_ord_item(sqlContext, val_fecha_ejecucion)
    
    df_otc_t_nc_list_values, val_str_df_otc_t_nc_list_values = fun_cargar_df_otc_t_nc_list_values(sqlContext)
    
    df_otc_t_vw_list_values, val_str_df_otc_t_vw_list_values = fun_cargar_df_otc_t_vw_list_values(sqlContext)
...
def fun_controlpoint2(sqlContext, df_otc_t_r_om_portin_co, df_otc_t_r_om_portin_co_mpn_sr, df_otc_t_r_ri_mobile_phone_number, df_otc_t_r_usr_users, df_otc_t_r_pmgt_store, df_otc_t_r_cim_res_cust_acct, df_otc_t_r_cim_bsns_cust_acct, df_otc_t_r_pim_cust_category, df_otc_t_r_ri_number_owner, df_otc_t_r_am_sim, df_otc_t_r_boe_sales_ord, df_otc_t_r_boe_ord_item, df_otc_t_nc_list_values, df_otc_t_vw_list_values):
    ...
    df_sol_port_in, val_str_cargar_df_sol_port_in = fun_cargar_df_sol_port_in(sqlContext, df_otc_t_r_om_portin_co, df_otc_t_r_om_portin_co_mpn_sr, df_otc_t_r_ri_mobile_phone_number, df_otc_t_r_usr_users, df_otc_t_r_pmgt_store, df_otc_t_r_cim_res_cust_acct, df_otc_t_r_cim_bsns_cust_acct, df_otc_t_r_pim_cust_category, df_otc_t_r_ri_number_owner, df_otc_t_r_am_sim, df_otc_t_r_boe_sales_ord, df_otc_t_r_boe_ord_item, df_otc_t_nc_list_values, df_otc_t_vw_list_values)
    
    val_str_resultado_cp2 = val_str_cargar_df_sol_port_in + "\n"  
    
    return val_str_resultado_cp2, df_sol_port_in
...
def fun_cargar_df_otc_t_r_om_portin_co(sqlContext, val_fecha_ejecucion):
...
def fun_cargar_df_otc_t_r_boe_sales_ord(sqlContext, val_fecha_ejecucion):
...
def fun_cargar_df_otc_t_r_boe_ord_item(sqlContext, val_fecha_ejecucion):
...
def fun_cargar_df_sol_port_in(sqlContext, df_otc_t_r_om_portin_co, df_otc_t_r_om_portin_co_mpn_sr, df_otc_t_r_ri_mobile_phone_number, df_otc_t_r_usr_users, df_otc_t_r_pmgt_store, df_otc_t_r_cim_res_cust_acct, df_otc_t_r_cim_bsns_cust_acct, df_otc_t_r_pim_cust_category, df_otc_t_r_ri_number_owner, df_otc_t_r_am_sim, df_otc_t_r_boe_sales_ord, df_otc_t_r_boe_ord_item, df_otc_t_nc_list_values, df_otc_t_vw_list_values):
    ...
    ###---list_nclv = list(df_otc_t_nc_list_values.select('codigo').toPandas()['codigo'])
    ...
    df_sol_port_in_tmp = t_a.join(t_mpn, expr(" mpn.object_id = a.object_id ") , how='left').\
        ...
        selectExpr('case when cr.cust_acct_number is null then cb.cust_acct_number else cr.cust_acct_number end as CustomerAccountNumber', 'case when cr.doc_number is null then cb.doc_number else cr.doc_number end doc_number', 'case when nclv2.value is null then nclv.value else nclv2.value end as doc_type','ctg.name as CustomerCategory',' a.object_id as PortinCommonOrderID','lv.value as RequestStatus','vwlv.localized_value as estado','a.fvc as fvc','donor.name as operadora','a.donor_account_type as donor_account_type1','lv3.value as LN_Origen','u.name as AssignedCSR','a.created_when as created_when','o.object_id as SalesOrderID','lv4.value as SalesOrderStatus','o.processed_when as SalesOrderProcessedDate','ri.name as telefono','substr(sim.name,1,19) as AssociatedSIMICCID','oi.tariff_plan_name as PlanDestino','a.ascp_rejection_comment  as motivo_rechazo')
    ...
    # Insercion de Datos
    val_retorno_insercion = fun_realizar_insercion_df_tabla_final(sqlContext, val_base_reportes, val_otc_t_solicitudes_port_in, df_sol_port_in)
